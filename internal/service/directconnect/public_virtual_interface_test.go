...
func TestAccDirectConnectPublicVirtualInterface_rateLimit(t *testing.T) {
	ctx := acctest.Context(t)
	connectionID := acctest.SkipIfEnvVarNotSet(t, "DX_CONNECTION_ID")
	...
	acctest.ParallelTest(ctx, t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccPublicVirtualInterfaceConfig_rateLimit(connectionID, rName, amazonAddress, customerAddress, bgpAsn, vlan, "1Gbps"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckPublicVirtualInterfaceExists(ctx, t, resourceName, &vif),
			...
			{
				Config: testAccPublicVirtualInterfaceConfig_rateLimit(connectionID, rName, amazonAddress, customerAddress, bgpAsn, vlan, "2Gbps"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckPublicVirtualInterfaceExists(ctx, t, resourceName, &vif),
					resource.TestCheckResourceAttr(resourceName, "rate_limit", "2Gbps"),
				),
			},
			{
				ResourceName:      resourceName,
...
func testAccPublicVirtualInterfaceConfig_rateLimit(cid, rName, amzAddr, custAddr string, bgpAsn, vlan int, rateLimit string) string {
	return fmt.Sprintf(`
resource "aws_dx_public_virtual_interface" "test" {
  address_family   = "ipv4"
  amazon_address   = %[3]q
...
`, cid, rName, amzAddr, custAddr, bgpAsn, vlan, rateLimit)
}
